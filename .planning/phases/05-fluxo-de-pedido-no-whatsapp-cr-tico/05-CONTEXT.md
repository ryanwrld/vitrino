...
- **D-05 (constraint técnica, não decisão de produto):** O link `wa.me` do WhatsApp só aceita texto no parâmetro da mensagem — é **tecnicamente impossível** anexar uma imagem automaticamente via deep link, em qualquer plataforma (Android, iOS, WhatsApp Web). Isso é uma limitação da API do WhatsApp, não uma limitação do código do Vitrino.
